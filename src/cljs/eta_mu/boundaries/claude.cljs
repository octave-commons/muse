       ...
       (str/join ",\n"
                 (map (fn [[claude-event script-name]]
                        (str "    " (js/JSON.stringify claude-event) ": [\n"
                             ...
                             "          {\n"
                             "            \"type\": \"command\",\n"
                             "            \"command\": \".claude/generated-hooks/" script-name ".sh\"\n"
                             "          }\n"
                             "        ]\n"
                             "      }\n"
                             "    ]"))
...
(defn- shell-word
  "Encode one literal POSIX shell word, including embedded apostrophes."
  [value]
  (str "'" (str/replace value "'" "'\"'\"'") "'"))

(defn hook-script-source
  "Render a wrapper that preserves the compiled entry path and event as literal arguments."
  [entry-path canonical-event]
  (str "#!/bin/bash\n"
       "# Generated by eta-mu.boundaries.claude/emit-hook-config! -- do not edit.\n"
       "# Canonical event: " canonical-event "\n"
       "exec node -- " (shell-word entry-path) " --hook " (shell-word canonical-event) "\n"))

(defn emit-hook-config!
  "Write .claude/settings.json + .claude/generated-hooks/*.sh for every canonical
  ...
  (let [canonical-events (map qualified-name (keys (:hooks adapter)))]
    (if (empty? canonical-events)
      ...
      (let [claude-events->script-names
        ...
        (.mkdirSync node-fs ".claude/generated-hooks" #js {:recursive true})
        (doseq [[script-name canonical-event] script-name->canonical
                :let [path (str ".claude/generated-hooks/" script-name ".sh")]]
