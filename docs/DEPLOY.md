...
| shadow-cljs | Installed at the pinned repo version by `npm install` | `node_modules/.bin/shadow-cljs --version` |
...
```sh
node_modules/.bin/shadow-cljs release daemon  # → dist-daemon/daemon.js
scripts/build-host-targets.sh all             # OpenCode + MCP + Claude, generator first
node_modules/.bin/shadow-cljs compile test    # sanity test target
```

The host-target script is the supported cold-checkout contract. It generates
each ignored `src/gen/eta_mu/gen/*` entry namespace from `.ημ/config` before
Shadow discovers the target's module graph. Pass one target instead of `all`
when only OpenCode, MCP, or Claude is needed.
...
2. Rebuild with `scripts/build-host-targets.sh opencode-plugin` (or touch a file under
...
| 3 | OpenCode can't load global plugin | Absolute-path shim from another checkout | Run `scripts/build-host-targets.sh opencode-plugin` locally |
